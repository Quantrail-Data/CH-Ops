...
import {
  isDataQuery as sqlIsDataQuery,
  leadingKeyword,
} from "../../shared/sqlClassify.js";
...
export async function executeQuery({
  ...
  const isExplainRaw =
    firstWord === "EXPLAIN" &&
    (/\bgraph\s*=\s*1/.test(lowerStripped) ||
      /\bjson\s*=\s*1/.test(lowerStripped));

  const fullSql =
    isDataQuery && !isExplainRaw ? trimmed + "\nFORMAT JSONEachRow" : trimmed;
...
export async function executeQueryWithBody({
  ...
  const fullQuery =
    jsonEachRow && !hasFormat ? `${trimmed} FORMAT JSONEachRow` : trimmed;
  ...
  url.searchParams.set("http_max_field_value_size", String(10485760));
