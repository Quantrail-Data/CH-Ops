...
async function classifyIntent(client, schemaContext, instruction) {
  ...
  if (intent !== "GREETING" && intent !== "DATABASE" && intent !== "OUT_OF_DOMAIN") {
...
export async function generateSql({
  jti, context, appUser, chatId = null, clusterId, node, tables,
  instruction, previousInstruction = null, previousSql = null,
  forceRefreshDdl = false,
}) {
  const { results: ddlBlocks, failures: ddlFailures } = await fetchDdl({
    jti, context, clusterId, node, tables, forceRefresh: forceRefreshDdl,
  });
  ...
  const promptParts = buildPrompt({
    instruction, ddlBlocks, previousInstruction, previousSql,
  });
  ...
  if (intent === "GREETING") {
    ...
    if (extracted === null || rawIfNoSql === "CANNOT_GENERATE_SQL") {
      responseText = "Unable to generate SQL for that question. Try adding more detail, or check the tables you've selected.";
    } else {
  ...
  if (chatId) {
    await appendMessage(appUser, chatId, {
      instruction, sql, responseText,
  ...
  return {
    chatId, instruction, sql, responseText, tokensEstimated,
    provider: cfg.provider, model: cfg.model, ddlFailures,
  };
}