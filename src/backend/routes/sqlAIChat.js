...
router.post("/generate-sql", async (req, res, next) => {
  try {
    const { database_ids, user_question } = req.body;


    if (
      !Array.isArray(database_ids) ||
      database_ids.length === 0 ||
      !user_question
    ) { 
      const error = new Error(
        "database_ids array and user_question must be included"
      );
      error.statusCode = 422;
      return next(error);
    }

    const targetDatabases = database_ids;
    ...
    const result = await service.generateSQL(targetDatabases, user_question);
