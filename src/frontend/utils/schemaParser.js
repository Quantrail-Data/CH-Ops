...
export function engineKind(engine) {
  ...
  if (engine === "View" || engine === "LiveView" || engine === "WindowView")
    return "view";
...
function parseDistributedTarget(createQuery) {
  ...
  const engineMatch = createQuery.match(
    /ENGINE\s*=\s*Distributed\s*\(\s*['"`]?(\w+)['"`]?\s*,\s*['"`]?(\w+)['"`]?\s*,\s*['"`]?(\w+)['"`]?\s*(?:,|[\s\)])/i,
  );
...
function parseViewSources(createQuery, defaultDb) {
  if (!createQuery || typeof createQuery !== "string") return [];
  ...
  while ((match = sourceRegex.exec(sql)) !== null) {
    const db = match[1] || match[2] || defaultDb;
  ...
  return sources.filter(({ db, table }) => {
...
export async function fetchSchemaData() {
  ...
  try {
    ...
    console.warn("columns:", e.message);
  }
  ...
  for (const d of dictsRes.rows)
    dictSources.set(tableKey(d.database, d.name), d.source);
  const refreshes = new Map();
  for (const r of refreshesRes.rows)
    refreshes.set(tableKey(r.database, r.view), r);
...
export function buildGraph(tables, columnsByTable, dictSources, refreshes) {
  ...
  for (const t of tables) {
    ...
    if (kind === "view" && t.create_table_query) {
      viewSources = parseViewSources(t.create_table_query, t.database);
    }
    ...
    const node = {
      ...
      dependsOn: [
        ...zipKeys(
          t.loading_dependencies_database,
          t.loading_dependencies_table,
        ),

        ...viewSources.map(({ db, table }) => tableKey(db, table)),
      ],

      columns: columnsByTable.get(key) || [],
      refresh: refreshes.get(key),
      x: 0,
      y: 0,
      w: 0,
      h: 0,
  ...
  for (const node of nodes.values()) {
    ...
    if (isMv && node.targetTable) {
      addEdge(
        node.key,
        tableKey(node.targetDatabase || node.database, node.targetTable),
        "mv",
      );
    }
...
export async function fetchViewsLoad(nodes, loadPeriodDays) {
  ...
  for (const r of rows) {
    ...
    if (targetKey) {
      ...
      for (const m of METRIC_KEYS)
        maxByEdge[m] = Math.max(maxByEdge[m], eAgg[m]);
    }
  }
