    ...
    <form onSubmit={submit} className="card" style={{ padding: 20 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 14 }}>
        ...
        >
          <input
            type="checkbox"
            checked={f.dropAllSettings}
            onChange={(e) => u("dropAllSettings", e.target.checked)}
        ...
        >
          <input
            type="checkbox"
            checked={f.dropAllProfiles}
            onChange={(e) => u("dropAllProfiles", e.target.checked)}
            style={{ accentColor: "var(--accent)" }}
            disabled={!isAdmin}
          />{" "}
          DROP ALL PROFILES
...
function DropRole({ rbac, roles, clusters, setResult, onSuccess }) {
  ...
  return (
    <div
      className="card"
      style={{ padding: 20, height: confirm ? "600px" : "auto" }}
    >
      <div
        style={{
          display: "grid",
      ...
      {confirm && (
        <ConfirmModal
          ...
          message={
            <div>
              <p>
                Type the role name <strong>{sel}</strong> to confirm:
              </p>
              <input
                className="form-input"
                style={{ marginTop: 8 }}
