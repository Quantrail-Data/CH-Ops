...
export default function UserManagement() {
  ...
  const [changePw, setChangePw] = useState({
    show: false,
    current: { value: "", isView: false },
    newPw: { value: "", isView: false },
    confirm: { value: "", isView: false },
  });
  ...
  async function sendTestEmail() {
    if (!testTo.trim()) {
      toast.warning("Enter an address to send to.");
      return;
    }
    setSmtpBusy(true);
    setSmtpResult(null);
  ...
  function smtpFormReady() {
    ...
    if (smtpForm.user?.trim() && !smtpForm.password && !smtp?.hasPassword)
      return false;
  ...
  function creatableRoles() {
    if (myRole === "superadmin")
      return ["superadmin", "admin", "editor", "readonly"];
  ...
  async function confirmRoleChange() {
    ...
    try {
      await apiFetch(`/api/users/${roleChange.userId}`, {
        method: "PUT",
        body: JSON.stringify({ role: roleChange.toRole }),
      });
      toast.success(
        `Role changed from ${roleChange.fromRole} to ${roleChange.toRole}.`,
      );
      load();
    } catch (err) {
      toast.error(err.message);
  ...
  async function deleteUser(id) {
    try {
      await apiFetch(`/api/users/${id}`, { method: "DELETE", body: {} });
  ...
  async function selfChangePassword(e) {
    ...
    try {
      await apiFetch("/api/auth/change-password", {
        method: "POST",
        body: JSON.stringify({
          currentPassword: changePw.current.value,
          newPassword: changePw.newPw.value,
        }),
      });
  ...
  const tableWrapStyle =
    users && users.length > 0 ? { minHeight: 0, paddingBottom: 0 } : {};
  ...
  return (
    <div className="page-content">
        ...
        <>
              ...
              <form onSubmit={selfChangePassword}>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setChangePw((p) => ({
                          ...p,
                          current: { ...p?.confirm, value: e.target?.value },
                        }))
                      }
                    />
                    <div
                      ...
                      onClick={() =>
                        setChangePw({
                          ...
                          current: {
                            ...changePw?.current,
                            isView: !changePw?.current?.isView,
                          },
                        })
                      }
                    >
                      {changePw?.current?.isView ? (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setChangePw((p) => ({
                          ...p,
                          newPw: { ...p?.newPw, value: e.target?.value },
                        }))
                      }
                    />
                    <div
                      ...
                      onClick={() =>
                        setChangePw({
                          ...
                          newPw: {
                            ...changePw?.newPw,
                            isView: !changePw?.newPw?.isView,
                          },
                        })
                      }
                    >
                      {changePw?.newPw?.isView ? (
                        <Icon className="ti ti-eye-off" />
                      ) : (
                        <Icon className="ti ti-eye" />
                      )}
                ...
                <div className="form-group" style={{ marginBottom: 16 }}>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setChangePw((p) => ({
                          ...p,
                          confirm: { ...p?.confirm, value: e.target?.value },
                        }))
                      }
                    />
                    <div
                      ...
                      onClick={() =>
                        setChangePw({
                          ...
                          confirm: {
                            ...changePw?.confirm,
                            isView: !changePw?.confirm?.isView,
                          },
                        })
                      }
                    >
                      {changePw?.confirm?.isView ? (
                        <Icon className="ti ti-eye-off" />
                      ) : (
                        <Icon className="ti ti-eye" />
                      )}
                  ...
                  {changePw.newPw.value &&
                    changePw.confirm.value &&
                    changePw.newPw.value === changePw.confirm.value && (
                      <div
                        style={{
                          marginTop: 8,
                          display: "flex",
          ...
          {!isSmtpConfigured && (
            <div className="alert-banner info" style={{ marginBottom: 14 }}>
              ...
              retrieve passwords directly in the UI.
            </div>
          )}
          {generatedPw && (
            <div className="alert-banner success" style={{ marginBottom: 14 }}>
              <Icon className="ti ti-key"></Icon> Generated password:{" "}
              <strong style={{ fontFamily: "var(--font-code)", marginLeft: 8 }}>
                {generatedPw}
              </strong>{" "}
          ...
          {showCreate && isAdmin && (
            ...
            >
              ...
              >
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="For password email"
                  />
                </div>
                <div className="form-group">
              ...
              >
                A random password will be generated for first login. If email is
      ...
      {tab === "smtp" && smtpForm && (
        ...
        >
          <div style={{ marginBottom: 24 }}>
            ...
            >
              <Icon className="ti ti-mail" style={{ color: "var(--accent)" }} />
              System email
            </h3>
            ...
            >
              Used for new account credentials and password reset codes.
              Separate from the SMTP on alert channels.
            </p>
          </div>
          ...
          {smtpResult && (
            <div
              className={
                smtpResult.ok ? "alert-banner info" : "alert-banner danger"
              }
              style={{
                marginTop: 16,
                fontSize: 12,
            ...
            >
              <Icon
                className={
                  smtpResult.ok ? "ti ti-check" : "ti ti-alert-triangle"
                }
              />
          ...
          >
            ...
            <p style={{ margin: "0 0 8px" }}>
              Delete the configuration above to fall back to the SMTP_* values
              in the server environment. This takes effect immediately, with no
              restart.
            </p>
      ...
      {roleChange && (
        <ConfirmModal
          title="Change Role"
          message={`Change "${roleChange.username}" from ${roleChange.fromRole} to ${roleChange.toRole}?`}
