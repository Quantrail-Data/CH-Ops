    ...
    <div className="page-content">
      <div className="section-header">
        <h2 className="section-title">
          <Icon className="ti ti-shield-lock"></Icon> Session Log
        </h2>
      </div>
      ...
      {routeTab === "overview" && (
        <SessionLogOverview unavailable={conn.unavailable} />
      )}
      {routeTab === "search" && (
        <SessionLogSearch unavailable={conn.unavailable} />
      )}
    </div>
  );
}
...
async function runQueryProbe() {
  const res = await runQuery(
    "SELECT name FROM system.columns WHERE database = 'system' AND table = 'session_log'",
  );
...
function donutOption(byType) {
  const map = new Map(
    (byType || []).map((r) => [String(r.type), Number(r.cnt) || 0]),
  );
  const data = [];
  ...
  for (const r of byType || [])
    if (!TYPE_ORDER.includes(String(r.type)))
      data.push({ name: String(r.type), value: Number(r.cnt) || 0 });
  return {
    tooltip: {
      trigger: "item",
      confine: true,
...
function buildRateSeries(rows, from, to, rounding) {
  ...
  const fromSec = from
    ? Math.floor(new Date(from.replace(" ", "T")).getTime() / 1000)
    : 0;
  const toSec = to
    ? Math.floor(new Date(to.replace(" ", "T")).getTime() / 1000)
    : 0;
  ...
  const legendItems = names.map((name, i) => ({
    name,
    color: TYPE_COLOR[name] || PALETTE[i % PALETTE.length],
  }));
...
function BarCard({
  ...
  return (
    <ChartCard
      key={`${title}-${themeKey}`}
      title={title}
      height={Math.max(minHeight, rows.length * 26 + 40)}
      scrollToHeight={scrollToHeight}
      option={barOption(rows, labelWidth)}
    />
  );
}
...
function RateChart({ rows, from, to, rounding }) {
  ...
  const tools = useChartTools(() => chartRef.current, {
  ...
  useEffect(() => {
    ...
    return () => {
      disposeChart(containerRef.current);
...
function SessionLogOverview({ unavailable }) {
  ...
  const load = useCallback(
    async (override = {}) => {
      ...
      if (next.exists) {
        ...
        try {
          const res = await runQuery(
            `SELECT count() AS total, countIf(type = 'LoginSuccess') AS success, countIf(type = 'LoginFailure') AS failure, countIf(type = 'Logout') AS logout, uniqExact(user) AS users, max(event_time) AS last_seen FROM system.session_log ${where}`,
          );
        ...
        try {
          const res = await runQuery(
            `SELECT type, count() AS cnt FROM system.session_log ${where} GROUP BY type ORDER BY cnt DESC`,
          );
          next.byType = res.rows || [];
        } catch (err) {
          e.byType = err.message || "Query failed";
        }

        try {
          const res = await runQuery(
            `SELECT user, count() AS cnt FROM system.session_log ${where} GROUP BY user ORDER BY cnt DESC LIMIT 15`,
          );
          next.users = res.rows || [];
        } catch (err) {
          e.users = err.message || "Query failed";
        }

        try {
          const res = await runQuery(
            `SELECT interface, count() AS cnt FROM system.session_log ${where} GROUP BY interface ORDER BY cnt DESC LIMIT 15`,
          );
          next.iface = res.rows || [];
        } catch (err) {
          e.iface = err.message || "Query failed";
        }

        try {
          const res = await runQuery(
            `SELECT auth_type, count() AS cnt FROM system.session_log ${where} GROUP BY auth_type ORDER BY cnt DESC LIMIT 15`,
          );
          next.auth = res.rows || [];
        } catch (err) {
          e.auth = err.message || "Query failed";
        }

        try {
          const res = await runQuery(
            `SELECT toStartOfInterval(event_time, INTERVAL ${r} SECOND)::INT AS t, type, count() AS v FROM system.session_log ${where} GROUP BY t, type`,
          );
          next.rate = res.rows || [];
        } catch (err) {
          e.rate = err.message || "Query failed";
        }

        try {
          const res = await runQuery(
            `SELECT failure_reason, count() AS count, argMax(user, event_time) AS last_user, argMax(toString(client_address), event_time) AS last_client, max(event_time) AS last_seen FROM system.session_log ${where} AND type = 'LoginFailure' AND failure_reason != '' GROUP BY failure_reason ORDER BY count DESC LIMIT 50`,
          );
          next.table = res.rows || [];
        } catch (err) {
          e.table = err.message || "Query failed";
        }
      }
  ...
  const userRows = (data.users || []).map((r) => ({
    name: r.user == null || r.user === "" ? "(unknown)" : String(r.user),
    total: Number(r.cnt) || 0,
  }));
  const ifaceRows = (data.iface || []).map((r) => ({
    name:
      r.interface == null || r.interface === ""
        ? "(unknown)"
        : String(r.interface),
    total: Number(r.cnt) || 0,
  }));
  const authRows = (data.auth || []).map((r) => ({
    name:
      r.auth_type == null || r.auth_type === ""
        ? "(none)"
        : String(r.auth_type),
    total: Number(r.cnt) || 0,
  }));
  ...
  const getUnavailableMessage = () => {
    const match = unavailable.find(
      (item) => item.table === "system.session_log",
    );
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>
            ...
            logging is enabled in the server config, so an absent table means no
            login/logout events are being recorded.
          </p>
        </div>
      ) : !hasData && !hasErrs ? (
        <div className="empty-state">
          <Icon
            ...
            style={{ color: "#34d399" }}
          ></Icon>
          <p>No session events recorded in the selected range.</p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          >
            ...
            <Stat
              ...
              value={fmtInt(success)}
              icon="ti-circle-check"
              color="var(--color-success)"
            />
            <Stat
              label="Failed Logins"
              value={fmtInt(failure)}
          ...
          >
            {errs.byType ? (
              ...
              <ChartCard
                ...
                height={300}
                option={donutOption(data.byType)}
                chartType="pie"
              />
            )}
            {errs.users ? (
...
function SessionLogSearch({ unavailable }) {
  ...
  async function handleSearch(e) {
    ...
    if (reasonText.trim())
      conds.push(`failure_reason LIKE '%${reasonText.trim()}%'`);
        ...
        <div className="card" style={{ padding: 20, marginBottom: 20 }}>
          <form onSubmit={handleSearch}>
            ...
            >
              ...
              <div className="form-group">
                <label className="form-label">User (text)</label>
                <input
